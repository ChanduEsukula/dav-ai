...
class OpenFDAFoodEnforcementClient:
    ...
    async def search_food_recalls(
        ...
        params = {
            "search": (
                f'product_description:"{query}" '
                f'OR reason_for_recall:"{query}" '
                f'OR recalling_firm:"{query}"'
            ),
            "limit": min(limit, 25),
        }
        ...
        logger.info(
            "openfda_food_request_started",
            extra={
                "event": "openfda_food_request_started",
        ...
        try:
            ...
            if response.status_code == 404:
                ...
                logger.info(
                    "openfda_food_request_completed",
                    extra={
                        "event": "openfda_food_request_completed",
                        "request_id": request_id,
                        "product_module": "FoodRadar",
                        "source_id": self.source["source_id"],
                        "endpoint": endpoint,
            ...
            logger.exception(
                "openfda_food_request_failed",
                extra={
                    "event": "openfda_food_request_failed",
                    "request_id": request_id,
                    "product_module": "FoodRadar",
                    "source_id": self.source["source_id"],
                    "endpoint": endpoint,
                    "query": query,
                    "duration_ms": duration_ms,
                    "error_category": "openfda_food_request_error",
                },
            )
            raise
